...
- **KI-3 首次运行 CLI 发现竞态（P1+P2 同族）**：
  - P1 欢迎向导「欢迎使用 AISC Workbench」初次运行环境步骤失败，报
    找不到 aisc cli（多版本以来间歇出现）；
  - P2 初次进入选择工作区页显示无 aisc cli 能力，点「重新检测」后恢复
    （同族，多次出现）。
  - 待查方向：安装器交接链路（NSIS→Workbench）启动时的 PATH/发现时序
    （G-18 PATH 项刚写入、五源发现的首次探测时机）、向导环境步骤与
    negotiate 的先后；"同会话重检即恢复"指向启动期竞态而非环境永久
    缺失。修复后需在干净环境复测安装→首跑全链路。
