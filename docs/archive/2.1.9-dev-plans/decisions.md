...
## 封存盖印（2026-09-07，2.1.10 D-6）

本目录设计的 **F1（mutagen SSH 工作区）已在 2.1.10 周期剥离并封存**——用户
裁决该方案为错误尝试。剥离计划与执行记录见
`docs/plans/2.1.10-dev-plans/f1-strip-plan.md`；F1 最后完整代码状态以 tag
`v2.1.9-dev` 为准。「本地代码 + 远端 runtime」的同步语义如需复活，从该 tag
提取参考；后续远程化（远端 FS API 模型）大概率吸收该需求。F2（宿主工具
MCP）不受影响，继续在产。
