...
// --- 2.1.9 T3c (R3): inferred attribution of watcher changes ---
describe("inferred attribution (2.1.9 T3c)", () => {
  /** A minimal Tab whose active pane is a live session of *agent*. */
  function liveTab(tabId: string, agent: "claude" | "codex" | "bash", state: string) {
    const paneId = `${tabId}-p1`;
    return {
      tabId,
      agent,
      title: agent,
      ...
      activePaneId: paneId,
      panes: { [paneId]: { sessionId: "s1", sessionState: state, exit: null } },
    } as never;
  }

  it("attributes changes to the live provider session; bash and no-session stay unattributed", async () => {
    ...
    // No session at all → unattributed, exactly as before.
    explorer.handleWorkspaceChanges([
      { relative_path: "none.txt", change_type: "created", kind: "file", revision: 1 },
    ]);
    expect(explorer.unattributed["none.txt"]).toBe("created");
    expect(explorer.inferred["none.txt"]).toBeUndefined();

    // Live claude session → inferred with agent; unattributed entry carries it.
    runtime.tabs = [liveTab("t-claude", "claude", "running")];
    runtime.activeTabId = "t-claude";
    explorer.handleWorkspaceChanges([
      { relative_path: "claude.txt", change_type: "created", kind: "file", revision: 2 },
    ]);
    expect(explorer.inferred["claude.txt"]).toEqual({ agent: "claude", change_type: "created" });
    expect(explorer.unattributed["claude.txt"]).toBeUndefined();
    const entry = explorer.unattributedEntries.find((e) => e.relative_path === "claude.txt");
    expect(entry?.inferred_agent).toBe("claude");

    // A live bash session must NOT attribute (user-driven writes).
    runtime.tabs = [liveTab("t-bash", "bash", "running")];
    runtime.activeTabId = "t-bash";
    explorer.handleWorkspaceChanges([
      { relative_path: "bash.txt", change_type: "created", kind: "file", revision: 3 },
    ]);
    expect(explorer.inferred["bash.txt"]).toBeUndefined();
    expect(explorer.unattributed["bash.txt"]).toBe("created");

    // An exited claude pane no longer attributes.
    runtime.tabs = [liveTab("t-done", "claude", "exited")];
    runtime.activeTabId = "t-done";
    explorer.handleWorkspaceChanges([
      { relative_path: "after.txt", change_type: "created", kind: "file", revision: 4 },
    ]);
    expect(explorer.inferred["after.txt"]).toBeUndefined();
    expect(explorer.unattributed["after.txt"]).toBe("created");
  });

  it("a later unattributed batch overwrites an earlier inferred entry (and vice versa)", async () => {
    const pinia = createPinia();
    setActivePinia(pinia);
    const explorer = useWorkspaceExplorerStore();
    const { useRuntimeStore } = await import("../runtime");
    const runtime = useRuntimeStore();
    explorer.workspace = "/ws2";

    runtime.tabs = [liveTab("t1", "codex", "running")];
    runtime.activeTabId = "t1";
    explorer.handleWorkspaceChanges([
      { relative_path: "x.md", change_type: "created", kind: "file", revision: 1 },
    ]);
    expect(explorer.inferred["x.md"]?.agent).toBe("codex");

    // Session ends; the file changes again → plain unattributed wins now.
    runtime.tabs = [];
    explorer.handleWorkspaceChanges([
      { relative_path: "x.md", change_type: "modified", kind: "file", revision: 2 },
    ]);
    expect(explorer.inferred["x.md"]).toBeUndefined();
    expect(explorer.unattributed["x.md"]).toBe("modified");
  });
});
