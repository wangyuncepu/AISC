...
use base64::Engine;
use tokio::sync::mpsc;
use workbench_lib::pty::{spawn_pty_session, PtyEvent};
...
async fn drain_until_exit(rx: &mut mpsc::Receiver<PtyEvent>, d: Duration) -> (String, Option<(Option<i32>, String)>) {
    ...
    while Instant::now() < deadline {
        match tokio::time::timeout(Duration::from_millis(500), rx.recv()).await {
            Ok(Some(PtyEvent::Output { bytes, .. })) => {
                out.push_str(&String::from_utf8_lossy(&decode(&bytes)));
            }
...
async fn echo_child_streams_output_and_exit_code() {
    ...
    let (out, exit) = drain_until_exit(&mut rx, Duration::from_secs(3)).await;
...
async fn write_input_is_echoed_and_close_is_user_close() {
    ...
    while Instant::now() < deadline {
        match tokio::time::timeout(Duration::from_millis(300), rx.recv()).await {
            Ok(Some(PtyEvent::Output { bytes, .. })) => {
                if String::from_utf8_lossy(&decode(&bytes)).contains("ping") {
                    got = true;
                    break;
                }
            }
...
async fn resize_does_not_error() {
    ...
    let _ = drain_until_exit(&mut rx, Duration::from_secs(2)).await;
    drop(session);
}
...
async fn real_aisc_session_open_bash() {
    ...
    while Instant::now() < deadline {
        match tokio::time::timeout(Duration::from_millis(500), rx.recv()).await {
            Ok(Some(PtyEvent::Output { bytes, .. })) => {
                if String::from_utf8_lossy(&decode(&bytes)).contains("hi_aisc") {
                    got = true;
                    break;
                }
            }
    ...
    assert!(got, "did not see hi_aisc");
